...
├── optimize_regime_grid_v2.py  Phase 3: 레짐 그리드 서치
├── calibrate_tqqq.py           TQQQ 비용 캘리브레이션
├── diag_nasdaq.py              NASDAQ 데이터 품질 진단
├── validate_eulb.py            eulb 결과 검증
├── analyze_crises.py           위기 분석
├── daily_signal_generator.py   일일 시그널 생성
├── daily_signal_telegram.py    텔레그램 알림
...
│
├── docs/                       분석 보고서 & 가이드
│   ├── CRITICAL_REVIEW.md          코드 리뷰
│   ├── VALIDATION_REPORT.md        Walk-forward 검증 결과
│   ├── LAG_CORRECTION_FINAL_REPORT.md  Part 7-9 보정 분석
│   ├── DAILY_SIGNAL_SETUP.md       일일 시그널 자동화 가이드
│   └── ...
│
├── tests/                      검증 & 수정 스크립트
│   ├── test_lag_comparison.py      Look-ahead bias 검증
│   ├── test_walk_forward.py        Walk-forward IS/OOS 비교
│   ├── test_vol_percentile_fix.py  변동성 퍼센타일 버그 검증
│   ├── fix_part79_lag_mismatch.py  Part 7-9 보정 테이블 생성
│   └── ...
│
├── archive/                    레거시 스크립트
